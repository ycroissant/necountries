...
**sf** [@PEBE:18], **ggplot2** [@WICK:16], and some other **R**
...
These two `sf` are exported by **necountries** and can therefore be used
directly. 
...
palette can be used using the `palette` argument. For example, to use the `"PuOr"` divergent
palette:
...
slave_trade <- slave_trade %>%
    mutate(country =
       case_when(country == "Democratic Republic of Congo" ~ "D.R. Congo",
...
```{r}
#| eval: true
strade <- countries("Africa", capital = TRUE) %>%
